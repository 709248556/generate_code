...
<!DOCTYPE html>
	...
	<script type="text/javascript">
      //将当前操制器名赋到js变量
      var newaction = "customerAddressIndex";
    </script>
	</head>
	<body>
		<div class="main_con">
		...
		<div class="search_box">
			<form method="get" id="SearchF" action="${ctx}/customer/address">
						...
						<table>
							<tbody>
								<tr>//3
                                    ...
                                    <th width="100" class="change"></th>
                                    <th width="100" class="change"></th>
                                    <th width="100" class="change">街道编号</th>
                                    <th width="100" class="change">街道名称</th>
                                    <th width="100" class="change">所属社区</th>
                                    <th width="100" class="change">详细地址</th>
                                    <th width="100" class="change">默认地址</th>
                                    <th width="100" class="change"></th>
                                    <th width="100" class="change"></th>
                                    <th width="100" class="change"></th>
								</tr>
				...
				<div class="tablelist_tbody" data-mcs-theme="dark">
					<table>
						<tbody>
							...
							<tr>//4
                                <td class="notitle">
                                    <input name="Item_Check" type="checkbox" value="${entity.custAddrId}" />
                                </td>
                                <td>
									<span>
										<a href='javascript:void(0)' class="detail-btn" onClick="detail('${entity.custAddrId}')">详情</a>
									</span>
                                </td>
								<td>${entity.custAddrId}</td>
								<td>${entity.customerId}</td>
								<td>${entity.contactsName}</td>
								<td>${entity.contactsPhone}</td>
								<td>${entity.contactsGender}</td>
								<td>${entity.provinceCode}</td>
        ...
        function deleteCustomerAddress() {
            ...
            if(id){
                parent.layer.confirm('您确定要删除选择的数据吗?该数据删除后不可恢复！请谨慎操作!', {
                    ...
                    AjaxConfirm("post","${ctx}/customer/address/deleteCustomerAddress?id=" + id);
                });
            }
        }
